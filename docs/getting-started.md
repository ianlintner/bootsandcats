...
- **Docker** (optional, for containerized deployment and testcontainers tests)
- **PostgreSQL 16** (for production, H2 is used for development)
...
1. Generate a new EC JWK (includes private key):
    ...
    ```bash
    ./gradlew :server-ui:run --args='--spring.main.web-application-type=none --generate-jwk' > jwk.json
    ```

    Or use the script:

    ```bash
    ./scripts/generate-ec-jwk.sh > jwk.json
    ```
