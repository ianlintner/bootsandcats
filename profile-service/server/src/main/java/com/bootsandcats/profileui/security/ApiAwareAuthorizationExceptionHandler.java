...
/**
 * Custom authorization exception handler that returns 401/403 for API endpoints instead of
 * redirecting to OAuth2 login.
 *
 * <p>This handler is designed for bearer token authentication (JWT) and does not require OAuth2
...
public class ApiAwareAuthorizationExceptionHandler
        implements io.micronaut.http.server.exceptions.ExceptionHandler<
                AuthorizationException, MutableHttpResponse<?>> {

    private static final Logger LOG =
            LoggerFactory.getLogger(ApiAwareAuthorizationExceptionHandler.class);
