 ...
 *
 * <p>This class provides a complete mock of Google's OIDC endpoints: - GET
 * /.well-known/openid-configuration - Discovery document - POST /token - Token endpoint - GET
 * /userinfo - User info endpoint - GET /oauth2/v3/certs - JWKS endpoint
 *
 ...
 *
 * <ul>
 *   <li>Google uses standard OIDC with 'sub' as the user identifier
 *   <li>Discovery document must be available for Spring Security to auto-configure
 *   <li>JWKS endpoint must return valid keys for ID token validation
 *   <li>If token validation fails, check the JWKS keys match the ID token signature
 * </ul>
 */
public class MockGoogleOidcServer {
     ...
     *
     * <p>Returns an access token and ID token. Note: The ID token is a placeholder since proper JWT
     * signing requires more infrastructure.
     */
        ...
        wireMockServer.stubFor(
                post(urlPathEqualTo("/token"))
                        .willReturn(
                                ...
                                """
                                                        .formatted(
                                                                TEST_ACCESS_TOKEN, mockIdToken))));
    }

    /**
     * Creates a mock ID token for testing.
     *
     * <p>Note: This is a simplified JWT structure for testing OAuth2 flows. For production-grade
     * testing with proper JWT validation, consider using a library like Nimbus JOSE+JWT.
     */
    private String createMockIdToken() {
        ...
        String signature =
                Base64.getUrlEncoder().withoutPadding().encodeToString("mock-signature".getBytes());
