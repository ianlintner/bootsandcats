 ...
 *
 * <p>This class provides a complete mock of GitHub's OAuth2 endpoints:
 * - POST /login/oauth/access_token - Token endpoint
 * - GET /user - User info endpoint
 * - GET /user/emails - User emails endpoint
 *
 * <p>Used for testing federated identity flows without hitting real GitHub APIs.
 *
 * <h2>AI Agent Debugging Notes:</h2>
 * <ul>
 *   <li>If tests fail with connection refused, ensure WireMock server is started</li>
 *   <li>Check that OAuth2 client registration URLs point to WireMock server</li>
 *   <li>Verify mock responses match expected GitHub response formats</li>
 *   <li>GitHub uses 'id' as the user identifier, 'login' as username</li>
 * </ul>
 */
public class MockGitHubOAuthServer {
    ...
    public static final String TEST_USER_AVATAR = "https://avatars.githubusercontent.com/u/12345678";
     ...
     *
     * <p>GitHub returns tokens in either JSON or URL-encoded format based on Accept header.
     * Spring Security uses JSON format.
     */
    private void stubTokenEndpoint() {
        wireMockServer.stubFor(post(urlPathEqualTo("/login/oauth/access_token"))
        ...
        wireMockServer.stubFor(post(urlPathEqualTo("/login/oauth/access_token"))
                ...
                .willReturn(aResponse()
                        ...
                        .withBody("access_token=%s&token_type=bearer&scope=read%%3Auser%%2Cuser%%3Aemail"
                                .formatted(TEST_ACCESS_TOKEN))));
    }
     ...
     *
     * <p>Returns user profile data that Spring Security OAuth2 client parses
     * for federated identity information.
     */
    ...
    private void stubUserEmailsEndpoint() {
        wireMockServer.stubFor(get(urlPathEqualTo("/user/emails"))
                .withHeader("Authorization", equalTo("Bearer " + TEST_ACCESS_TOKEN))
                .willReturn(aResponse()
                        .withStatus(200)
                        .withHeader("Content-Type", "application/json")
        ...
        wireMockServer.stubFor(get(urlPathEqualTo("/user"))
                ...
                .willReturn(aResponse()
                        ...
                        .withBody("""
                                ...
                                """.formatted(userId, login, name, email, userId))));
    }
